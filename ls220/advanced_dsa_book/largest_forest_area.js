...
function largestForestArea(grid) {
  ...
  function exploreForest(row, col) {
    ...
    if (col - 1 >= 0) exploreForest(row, col - 1);
    if (row - 1 >= 0) exploreForest(row - 1, col);
    if (col + 1 < grid[0].length) exploreForest(row, col + 1);
    if (row + 1 < grid.length) exploreForest(row + 1, col);
  }
