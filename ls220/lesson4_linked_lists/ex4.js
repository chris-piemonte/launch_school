...
output: head of linked list with all odd nodes grouped todgether in beginning, even nodes grouped together at end, in same order

rules:
- odd nodes & even node groups need to stay in same order
- the first node is odd
- if list empty or 1 node, return original list

---DATA---
- pointers for odd & even nodes to iterate through list and connect nodes of their kind
- dummy node to point to head and reattach end of odd list to beginning of even list

---ALGORITHM---
- if head or the 2nd node is missing, return head

- create an odd pointer at head and an even pointer at the 2nd node

- until the even pointer's node or next node is null
  - set the odd pointer's next to the even's next
  - set the even pointer's next to 2 further nodes

- sssign the last odd node's next to the 2nd node in the list using dummy or head
